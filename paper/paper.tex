...
Step 4 samples exactly from \(p(w_t \mid y_t, u)\) and step 3 resamples using the exact predictive weights, so this is the fully adapted auxiliary particle filter for this model. Under multinomial resampling \(\prod_t \frac1N\sum_i \alpha^{(i)}\) is an unbiased estimator of \(P(y_1^n)\) \cite[Prop.~7.4.1]{DelMoral2004}; we use systematic resampling for its lower variance, which forgoes this exact unbiasedness but leaves intact the convergence from above documented in Section 5. The computational cost is \(O(nNL)\) time and \(O(NL)\) memory. Numerically stable evaluation of the cell probabilities and of the truncated-normal draws requires some care in log space; the details are given in Appendix A and in the accompanying code.
