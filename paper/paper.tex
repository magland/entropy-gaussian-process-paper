...
The estimator overestimates \(\bar H\) in expectation for two reasons. First, by Jensen's inequality, \(\mathbb E\log\hat P(y_1^n) \le \log P(y_1^n)\), since the particle likelihood is unbiased on the natural scale but not on the log scale; the resulting bias is of order \(O(1/N)\) per sample; Section 5.1 observes this rate for processes of moderate difficulty (for the hardest processes the decay is slower at practical particle counts), and it is an observation rather than a bound we have proved. Second, the finite-\(n\) block entropy \(\tfrac1n H(Y_1^n)\) is non-increasing in \(n\) and converges to \(\bar H\) \cite{CoverThomas2006}. Both effects vanish in the appropriate limit, so that \(\hat{\bar H}\) approaches \(\bar H\) from above and the practical convergence check is to increase \(N\) and \(n\) until the estimate stops decreasing. Note that the two limits cannot be interchanged: at fixed \(N\) the Jensen gap is a per-sample bias that does not diminish as \(n\) grows, so the particle count must be increased along with the sequence length. Results are reported as the mean \(\pm\) standard error over independent replicates, each with a freshly generated sequence and filter, and Section 5 includes a convergence study with respect to both \(N\) and \(n\).
...
The estimator of Section 4 has two convergence parameters, the particle count \(N\) and the sequence length \(n\), and the experiments in this subsection establish how they should be chosen. Seven processes are used for the convergence studies, chosen to span the range of difficulty the filter encounters, all quantized at \(\Delta = 0.25\sigma\), that is, with a signal standard deviation of four quantization steps, which is representative of the target application: moving averages of widths 8, 16, 32, and 64, Gaussian smoothing kernels of widths \(\tau = 1\) and \(\tau = 1.25\), and the first difference. Two strongly smoothing processes, for which the filtering problem is hardest, are used to study the collapse diagnostic: a 33-tap windowed-sinc lowpass filter (cutoff 6 kHz at a sampling rate of 30 kHz) at \(\Delta = 0.25\sigma\) (\(\Delta/h_0 = 6.1\)), and a Gaussian kernel of width \(\tau = 1.5\) at \(\Delta = 0.5\sigma\) (\(\Delta/h_0 = 8.6\); the coarser step places the collapse transition within the range of particle counts studied). All runs used the WebGPU engine described in Section 6, and scripts reproducing every figure and table, including the seeds, are included in the repository.

Figure 1 shows the paired excess of the estimate at particle count \(N\) over the estimate at \(N = 128000\) on the same observed sequences, at fixed \(n = 20000\) with 32 replicates. The same seeds, and therefore identical observed sequences, are used at every \(N\), so that only the internal randomness of the filter differs along each curve, and the difference against the reference cancels the sequence-to-sequence variation and isolates the filter bias. The excess is positive throughout and decreases with \(N\) in every process, as expected from the Jensen bias discussed in Section 4.4. For the processes of moderate difficulty the decay is consistent with the expected \(O(1/N)\) rate over more than two decades: for the Gaussian kernel with \(\tau = 1.25\) the excess falls from \(20.5 \pm 0.4\) millibits at \(N = 125\) to \(0.15 \pm 0.03\) at \(N = 16000\), a factor of 137 over a 128-fold range of \(N\), and the \(\tau = 1\) kernel and the first difference behave in the same way at an amplitude roughly ten times smaller.
...
The size of the bias at a given \(N\) spans more than two orders of magnitude across the processes: at \(N = 125\) the excess is \(1.4 \pm 0.1\) millibits for the first difference and \(244 \pm 7\) millibits for the moving average of width 64. This variation is evidently not governed by the dimension of the state: the AR(1) process with \(\rho = 0.9\), whose filter carries a 63-dimensional state, is converged to within a fraction of a millibit already at \(N = 125\) (its paired excess, measured separately, is \(0.17 \pm 0.05\) millibits there and is unresolvable at larger \(N\), which is why it does not appear in the figure). It is instead related to the structure of the spectrum: the moving averages, whose spectra vanish at the \(w - 1\) nulls of the boxcar response, are the most biased processes at every width, and the bias grows systematically with the width. Furthermore, for the wide moving averages the decay is visibly slower than \(1/N\) over this range: the local exponent falls from about \(0.9\) at width 8 to about \(0.4\) at width 64, for which the excess drops only by a factor of six over the 128-fold range of \(N\). The \(O(1/N)\) asymptotics presumably still hold, but the particle count at which they set in recedes as the process hardens; this is the same slow convergence encountered at the wide moving averages in Section 5.2. In practice the convergence check of Section 4.4 (increase \(N\) until the estimate stops decreasing) is inexpensive, and for processes of moderate difficulty a few thousand particles suffice: at \(N = 2000\) the residual bias is below three millibits for every process shown except the moving averages of width 16 and wider.

Figure 2 shows the corresponding sweep over the sequence length, at fixed \(N = 4000\) for the moderate processes and \(N = 16000\) for the moving averages of widths 32 and 64, following the rule above. The replicate standard deviation shrinks in proportion to \(1/\sqrt{n}\) for every process across the 64-fold range of lengths, while the means are constant to within their error bars (for the moving average of width 8, \(2.7344 \pm 0.0037\) at \(n = 2500\) against \(2.7319 \pm 0.0005\) at \(n = 160000\)). The finite-\(n\) transient discussed in Section 4.4, which is \(O(L/n)\) of the total, is resolvable only for the widest kernel at the shortest lengths: the width-64 moving average reads \(1.6827 \pm 0.0100\) at \(n = 2500\), falling to \(1.6535 \pm 0.0031\) by \(n = 40000\) and constant thereafter. In other words, at practical lengths the role of \(n\) is to increase precision, while the bias is controlled by \(N\) alone. Since the variance of the reported mean scales as the reciprocal of the total sample count \(rn\), the division of a fixed budget between longer sequences and more replicates is largely a matter of convenience, provided \(n\) is much longer than the correlation length; independent replicates also furnish the error bar. Indeed, the experiments of Section 5.2 exploit this freedom in the opposite direction: since every step is an opportunity to lose lock, their hardest points are run as many short replicates rather than a few long ones.
...
For the two strongly smoothing processes the picture changes qualitatively. Figure 3 shows, as a function of the particle count, the fraction of steps flagged by the lost-lock diagnostic of Appendix B together with the mean effective sample size, and Table 1 reports the corresponding estimates (\(n = 30000\), eight replicates). At small \(N\) every replicate loses the state, and the resulting values are not merely inaccurate but invalid: about \(2.7 \times 10^5\) bits per sample for the Gaussian kernel at \(N = 300\), and about \(7 \times 10^8\) for the lowpass, in both cases far above the memoryless upper bound \(H(Y_1)\) (3.06 and 4.05 bits respectively), so that the bound check alone already rejects these runs. The transition to reliable operation is fairly sharp for the Gaussian kernel: every replicate fails at \(N = 300\), seven of eight fail at \(N = 1000\), and none fail at \(N = 3000\) or beyond. The lowpass process, whose state has dimension 32, is harder: every replicate fails through \(N = 3000\), five of eight at \(N = 10000\), and two of eight at \(N = 30000\); only at \(N = 100000\) do all replicates keep lock. Note that each step is an opportunity to lose the state, so the particle count required to keep every replicate locked grows with the sequence length.
...
The effective sample size does not detect these failures. Indeed, the mean ESS is higher in the collapsed regime (90 to 99 percent of \(N\)) than in the healthy one (31 and 42 percent for the two processes): once every particle is comparably far from the true state, the weights are nearly uniform and the resampling statistics appear ideal. The ESS is thus not merely insensitive to the failure but is actually larger in the failed runs than in the healthy ones, and no threshold on it can reject the collapsed runs. The per-step log-likelihood diagnostic, by contrast, flags the collapsed steps directly, and the affected replicates are discarded rather than averaged in.
...
An estimate for which some replicates were discarded should therefore be treated as provisional, and the working rule is to increase \(N\) until no replicate loses lock. Note that the surviving replicates at the provisional particle counts already agree with the fully locked estimate to within their error bars (2.1874, 2.1817, and 2.1806 for the lowpass at \(N = 10000\), \(30000\), and \(100000\)), so the discipline of discarding and rerunning costs little beyond computation. It is also reassuring that the converged estimates fall slightly below the analytical approximation (\(2.1806 \pm 0.0015\) against \(2.1882\) bits for the lowpass, and \(1.6353 \pm 0.0018\) against \(1.6415\) for the Gaussian kernel), which is the direction predicted by the analysis of Section 3.4.

\subsection{Accuracy of the analytical approximation}\label{accuracy-of-the-analytical-approximation}

The approximation of Section 3 was compared against the particle-filter estimate across six process families: four parameterized families (the moving average as a function of its width \(w\), the AR(1) process as a function of its correlation \(\rho\), the Gaussian kernel as a function of its width \(\tau\), and the windowed-sinc lowpass as a function of its cutoff), together with white noise and the first difference \([1, -1]\), whose spectrum vanishes at DC. The four parameterized families are shown in Figures 4 through 7, one figure per family, and all four figures have the same structure. Panel (a) shows the entropy rate as a function of the family parameter at four quantization steps, \(\Delta = \sigma/8\), \(\sigma/4\), \(\sigma\), and \(2\sigma\); each curve is the approximation evaluated on a dense parameter grid, and the open circles are the particle-filter estimates, computed at eight parameter values per curve, so that the approximation is checked at every part of each sweep rather than at isolated spot checks. Panel (b) shows the same comparison on the millibit scale, plotting the approximation minus the estimate at each measured point with error bars of one standard error; each error panel is scaled to its own family, so the vertical scales are not comparable between figures. White noise and the first difference have no family parameter and are treated separately in Figure 8, with the quantization step itself on the horizontal axis.
...
The filter parameters follow the convergence study of Section 5.1, with the particle count set from the one-step prediction error \(h_0\) of each point, since it is \(h_0\) that governs how hard the filtering problem is. Points with \(h_0 > 0.3\sigma\) use \(N = 8000\); those below use \(N = 30000\), and those with \(h_0 < 0.05\sigma\), namely the lowest four lowpass cutoffs, use \(N = 130000\). Sixteen replicates are run at every point, of length \(n = 10000\), reduced to \(n = 5000\) at the \(h_0 < 0.05\sigma\) points. The short sequences deserve a word of explanation. Each step is an opportunity to lose lock, so the particle count needed to keep a replicate locked grows with the sequence length, whereas the precision of the reported mean depends only on the total sample count \(rn\) (Section 5.1). Splitting a fixed budget into more and shorter replicates therefore buys the same precision at a much lower particle count: at the hardest lowpass point, sixteen replicates of length 5000 all keep lock at \(N = 130000\), while eight of length 20000 lose it six times at \(N = 30000\). We verified directly that the shortening introduces no finite-\(n\) bias, the two splits of a fixed budget agreeing to within one standard error (\(2.6615 \pm 0.0031\) at \(n = 5000\) with sixteen replicates against \(2.6581 \pm 0.0019\) at \(n = 20000\) with four, for the lowpass at cutoff \(0.2 f_s\) and \(\Delta = \sigma/8\)). Each point uses independently seeded sequences, so the scatter in the error panels is independent from point to point. The per-point parameters, together with the complete numerical values, are recorded in the tables of Appendix C. For white noise the symbol probabilities are available in closed form and the likelihood is computed exactly, so that comparison contains no filter error at all.

For \(\Delta \le \sigma\) the estimates fall on the curves throughout, across families, parameter values, and quantization steps, and the rate panels show no visible discrepancy. The error panels resolve what the rate panels cannot. Over most of each sweep the measured error is within a few millibits of zero: for the AR(1) family it lies between \(-4.4\) and \(+2.9\) millibits at \(\Delta = \sigma/8\) and \(\sigma/4\), and for the Gaussian family between \(-6.3\) and \(+3.5\), against standard errors of about 3 millibits, so most of this scatter is the noise of the estimator rather than error of the formula. Toward the strongly smoothing end of each sweep a systematic overestimate develops, in the direction predicted by the analysis of Section 3.4. It is clearest for the lowpass family, whose error grows from a few millibits at the wider cutoffs to \(+38.8\) millibits at cutoff \(0.08 f_s\) and \(\Delta = \sigma/8\), and \(+68.4\) millibits at \(\Delta = \sigma\), roughly five percent of the rate at that point. Restricted to cutoffs of \(0.2 f_s\) and above, the same family agrees to within \(13\) millibits at every step up to \(\sigma\).

At \(\Delta = 2\sigma\) the overestimate becomes systematic in every family and grows with the strength of the smoothing: about 12 to 15 millibits for white noise, the first difference, and the weakly correlated ends of the sweeps, rising to \(+70.7\) millibits for the AR(1) process at \(\rho = 0.95\), \(+79.7\) for the Gaussian kernel at \(\tau = 1.5\), and \(+148.2\) for the lowpass at its lowest cutoff, in the last case about 25 percent of the rate. This is the onset of the coarse-quantization failure anticipated in Section 3.5, and it is why the curves in the rate panels visibly separate from the estimates only at this step.

The moving average is a partial exception, and the one place in these experiments where the comparison is limited by the estimator rather than by the formula. Through width 8 it behaves like the other families, agreeing to within a few millibits for \(\Delta \le \sigma\). At the wider kernels and the finer steps, however, the estimate lies systematically \emph{above} the approximation, by \(35.6\) millibits at \(w = 32\) and \(158.3\) millibits at \(w = 64\) for \(\Delta = \sigma/8\). Since both methods are biased upward, a negative measured error of this size cannot be an error of the approximation, and must instead be residual bias of the filter. A separate sweep of the particle count at \(w = 64\) and \(\Delta = \sigma/8\) confirms this reading and shows how slow the convergence is: the estimate falls from \(2.4748 \pm 0.0120\) at \(N = 30000\) to \(2.4334 \pm 0.0054\) at \(N = 130000\) and \(2.4217 \pm 0.0051\) at \(N = 400000\), still \(112\) millibits above the approximation and still decreasing. This is consistent with the finding of Section 5.1 that the moving average is by far the most biased family for the filter, its spectrum vanishing at each of the \(w - 1\) nulls of the boxcar response, and it means that these particular points neither confirm nor contradict the approximation. Elsewhere in the sweep, and in every other family, the filter is converged and the comparison is meaningful.

Figure 8 carries the comparison for white noise and the first difference out to \(\Delta = 4\sigma\). The estimates track the curves to within about 5 millibits through \(\Delta = \sigma\) and fall below them by 13 to 14 millibits at \(2\sigma\). At \(4\sigma\) the failure is qualitative: the approximation returns \(0.658\) and \(0.623\) bits for the two processes while the estimates are \(0.314 \pm 0.002\) and \(0.292 \pm 0.003\), an error of about \(0.34\) bits, larger than the rate itself. Indeed, for white noise the approximation can also be checked against the exact marginal entropy, and its true error is below \(0.01\) millibits for \(\Delta \le \sigma\); the differences measured at the fine steps are therefore attributable to the estimator, not the formula, and they calibrate the residual scatter seen in the other families. As \(\Delta \to \infty\) the error approaches the constant floor \(\tfrac12\log_2(2\pi e/12) \approx 0.255\) bits discussed in Section 3.5, and it does so slowly from above, since the residual spectral term of the formula decays only as \(O(\sigma^2/\Delta^2)\) while the true rate vanishes faster.
...
The predictive pipeline comes within 2 to 4 percent of the entropy rate at \(\Delta = \sigma/4\) for every family (0.06 to 0.09 bits per sample of overhead), and its performance is similar at the finer steps. The gap widens as the quantization coarsens, reaching roughly 6 to 21 percent at \(\Delta = \sigma\), presumably because prediction from rounded values becomes less effective and the residual retains structure that the memoryless entropy coder cannot exploit. For white noise, where there is nothing to predict, ANS alone on the raw stream is within about one percent of the rate at every step; the extra cost visible in the table is the degenerate prediction stage of the uniform pipeline.

One detail of the pipeline deserves comment. The predictor order matters more than might be expected: with the seemingly natural choice of order \(L - 1\), matched to the length of the generating kernel, the MA(8) overhead at \(\Delta = \sigma/4\) is 0.32 bits per sample rather than 0.09, and the first difference behaves similarly. These are the families whose spectra have exact zeros, so their AR representations decay slowly and useful prediction requires an order well beyond the kernel length; at order 32 the excess is largely gone, and raising the order to 64 gains only about 0.02 bits more. The smooth families are insensitive to this choice.

The byte-oriented coder is far from the limit throughout: 35 to 89 percent above the rate at \(\Delta = \sigma/4\), and worst for the strongly smoothed families, which are precisely the ones with the most structure to exploit. Indeed, at this step zlib compresses the lowpass process to more bits than its memoryless entropy, so it fails to realize even the gains available to a symbol-by-symbol code. In other words, the compressibility of these signals resides almost entirely in the correlation structure (the order-0 entropy is the same 4.05 bits for every family), and a coder must model that structure explicitly to approach the limit. This is the motivation for the predictive pipeline, and for the codec study of the companion paper.
